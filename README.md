...
`prepareCopyRequest`, `prepareCopyResponse`, `validateCopySettingsRequest`, or
`importPlan`. It rejects unknown fields, duplicate JSON keys, unsupported
...
version's catalog contract. `sourceKey` identifies a source inside the integration;
there is no `resourceType`. `VariableValueSource` can optionally link the existing
filter selector to it with `SourceKey` and `ValueSemantics: "resource"` together.
The old `{type: "remote"}` declaration still works.

`ValidateDeclaration` checks source dependencies/cycles, referenced source keys,
required lookup/validator endpoints and constraints schemas. Transferable copy
rules explicitly declare `validation.mode` (`basic`, `integration`, or
`manualReview` with a message). A missing callback never falls back to manual
review. Objects reject unknown fields; omit applies only to declared derived
fields. Fields are optional
unless `required`; containers/templates can explicitly allow null with `nullable`.
`empty` on references distinguishes no selection, a required new selection, and
...
extra JSON. Private import plans require permission, revision/digest, expiration
and lifecycle checks in their owner; schema validation alone grants none of them.
