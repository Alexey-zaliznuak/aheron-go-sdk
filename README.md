...
Асимметричная, без общих секретов (см. `INTEGRATIONS.MD §5`):
...
- `Resolve` предпочитает абсолютный `resolve.url` из входящего payload, поэтому
  ...
  `ExecutionURL`.
- CRM ходит через префикс шлюза `/api/crm` (`/api/crm/*` → `crm-backend/api/*`).
  Если бьёте напрямую в crm-backend без шлюза — задайте `CRMURL` соответственно.
